  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="rounded-xl border border-zinc-200 bg-white p-6">
        ...
        <div className="mt-3 h-10 w-40 animate-pulse rounded bg-zinc-200" />
        <div className="mt-4 h-3 w-48 animate-pulse rounded bg-zinc-100" />
      </div>
...
function PayInvoiceModal({ cardId, month, amount, onClose, onSuccess, onError }: PayInvoiceModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await payInvoice(cardId, { accountId, month, amount, paymentDate });
    ...
    <>
      ...
      <div className="fixed inset-x-4 top-1/2 z-50 mx-auto max-w-sm -translate-y-1/2 rounded-2xl bg-white p-6">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-zinc-900">Pagar fatura</p>
            <p className="mt-0.5 text-sm text-zinc-500">
              Escolha a conta para debitar o pagamento.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-zinc-500">Conta de débito</label>
            <select
              value={accountId}
              onChange={(e) => setAccountId(e.target.value)}
              disabled={loading || accounts.length === 0}
              className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-900 focus:border-zinc-400 focus:outline-none disabled:opacity-50"
            >
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-zinc-500">Valor a pagar</label>
            <p className="text-lg font-light text-zinc-900">{formatCurrency(amount)}</p>
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-zinc-500">Data do pagamento</label>
            <p className="text-sm text-zinc-700">{formatDate(paymentDate)}</p>
          </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            {card ? (
              <>
                <h1 className="text-xl font-medium text-zinc-900">{card.name}</h1>
                {card.brand && <p className="mt-0.5 text-sm text-zinc-400">{card.brand}</p>}
              </>
            ) : (
              <>
                <div className="h-5 w-32 animate-pulse rounded bg-zinc-200" />
                <div className="mt-1 h-3.5 w-20 animate-pulse rounded bg-zinc-100" />
              </>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between rounded-xl border border-zinc-200 bg-white px-5 py-3">
          ...
          <span className="text-sm font-medium capitalize text-zinc-700">{monthLabel(month)}</span>
          <button
            onClick={handleNext}
            className="rounded-md p-1.5 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600"
        ...
        {invoiceLoading ? (
          ...
          <div className="rounded-xl border border-zinc-200 bg-white px-5 py-12 text-center">
            <p className="text-sm text-zinc-400">Não foi possível carregar a fatura.</p>
          </div>
        ) : invoice ? (
          <>
            {/* resumo */}
            <div className="rounded-xl border border-zinc-200 bg-white p-6">
              <p className="text-sm text-zinc-500">Total da fatura</p>
              <p className="mt-2 text-4xl font-light tracking-tight text-zinc-900">
              ...
              {invoice.totalAmount > 0 && (
                <button
                  ...
                  className="mt-5 w-full rounded-lg bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-zinc-800"
                >
                  Pagar fatura
                </button>
              )}
            ...
            {invoice.transactions.length === 0 ? (
              <div className="rounded-xl border border-zinc-200 bg-white px-5 py-12 text-center">
                <p className="text-sm text-zinc-400">Nenhuma compra nesta fatura.</p>
              </div>
            ) : (
              <div>
                <p className="mb-3 text-sm font-medium text-zinc-900">Compras</p>
                <div className="flex flex-col divide-y divide-zinc-100 rounded-xl border border-zinc-200 bg-white">
                  {invoice.transactions.map((tx) => (
                    <div key={tx.id} className="flex items-center justify-between gap-4 px-5 py-4">
                      ...
                      <span className="flex-shrink-0 text-sm tabular-nums text-zinc-500">
