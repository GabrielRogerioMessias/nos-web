  ...
  useEffect(() => {
    function handlePointerDown(e: PointerEvent) {
      if (
        ...
        !(e.target as Element)?.closest?.("[data-datepicker-popover]")
      ) {
        setOpen(false);
      }
    }
  ...
  const popover = open && mounted
    ? createPortal(
        <div
          ...
          className="z-[9999] rounded-xl border border-zinc-200 bg-white p-3 shadow-lg dark:border-zinc-700 dark:bg-zinc-900"
          style={popoverStyle}
        >
          <DayPicker
            mode="single"
            ...
            classNames={{
              ...
              selected:
                "bg-zinc-900 rounded-lg [&>button]:text-white [&>button]:hover:bg-zinc-800 dark:bg-zinc-100 dark:[&>button]:text-zinc-900 dark:[&>button]:hover:bg-zinc-200",
