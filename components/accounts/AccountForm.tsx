...
export function AccountForm({ editing, onSave, onCancel }: AccountFormProps) {
  ...
  return (
    <Modal
      ...
      footer={footer}
    >
      <form id="account-form" onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
        <Input
          label="Nome da conta"
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            disabled={!!editing}
            className={`w-full rounded-lg border px-3.5 py-2.5 text-sm outline-none transition-colors placeholder:text-zinc-400 focus:border-zinc-400 disabled:cursor-not-allowed disabled:text-zinc-400 dark:placeholder:text-zinc-600 dark:focus:border-zinc-500 dark:disabled:text-zinc-500 ${
              editing
          ...
          {editing && (
            <p className="text-xs text-zinc-400 dark:text-zinc-500">
              {editing.hasTransactions
                ? "O saldo inicial não pode ser alterado após o registro de transações."
                : "Para ajustar o saldo, crie uma transação de Ajuste."}
            </p>
          )}
        </div>
